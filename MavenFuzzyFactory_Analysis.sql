-- Задание 1.
-- Сравнить показатели отказов при 2 различных landing-page за исследуемый период.
...
-- Задание 3. 
-- Создать таблицу с данными о персональных скидках.
...
-- Задание 5. 
-- Проанализировать динамику суммы продаж и чистой прибыли по отдельным продуктам.

select 
	year(created_at) as yr,
    month(created_at) as mon,
    sum(case when t.product_id = 1 then t.price_usd else null end) as mrfuzzy_revenue,
    sum(case when t.product_id = 1 then t.price_usd - t.cogs_usd else null end) as mrfuzzy_marg,
    sum(case when t.product_id = 2 then t.price_usd else null end) as lovebear_revenue,
    sum(case when t.product_id = 2 then t.price_usd - t.cogs_usd else null end) as lovebear_marg,
    sum(case when t.product_id = 3 then t.price_usd else null end) as birthdaybear_revenue,
    sum(case when t.product_id = 3 then t.price_usd - t.cogs_usd else null end) as birthdaybear_marg,
    sum(case when t.product_id = 4 then t.price_usd else null end) as minibear_revenue,
    sum(case when t.product_id = 4 then t.price_usd - t.cogs_usd else null end) as minibear_marg,
    sum(t.price_usd) AS total_revenue,  
    sum(t.price_usd - t.cogs_usd) AS total_margin
from order_items t
group by 1, 2
order by 1, 2;